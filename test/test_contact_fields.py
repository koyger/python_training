...
def test_contact_fields_on_homepage_vs_edit_page(app):
    ...
    with allure.step('When I open edit page of contact "{}"'.format(homepage_contact.firstname)):
        edit_page_contact = app.contact.get_contact_info_from_edit_page(index)
    with allure.step('Then fields values should be the same as on contacts list page'):
...
def test_all_contact_fields_on_homepage_vs_db(app, db):
    with allure.step('Given two contact lists taken from UI and DB'):
    ...
    with allure.step('Each contact fields should be the same from both sources'):
