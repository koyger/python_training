...
def test_add_contact_to_group(app, orm_db):
    app.open_home_page()
    with allure.step('Given are existing contacts and groups'):
    ...
    with allure.step('Users and groups are prepared so it is possible to add user to group'):
    ...
    with allure.step('Then user "{}" is added to group "{}"'.format(user_to_add.firstname, group.name)):
    ...
    with allure.step('We check that user "{}" is added to group "{}"'.format(user_to_add.firstname, group.name)):
